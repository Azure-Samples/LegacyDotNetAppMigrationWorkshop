...
<ul>
    <li>Nginx supports ingress to Windows containers, but it must deployed on a Linux nodepool in the same cluster. </li>
    <li>Application Gateway supports ingress to Windows Containers, but it does not support Windows Authentication. For guidance on handling ingress with Windows Authentication and GMSA, refer to the [AKS Windows baseline](https://github.com/Azure/aks-baseline-windows).</li>
</ul>
